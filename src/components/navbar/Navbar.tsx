...
const Navbar = ({ className }: { className?: string }) => {
  ...
  // Navigation items based on service status
  const navItems = [
    { name: "Home", href: "/", alwaysShow: true },
    { name: "Flight", href: "/flight", enabled: services.flight },
    { name: "Hotel", href: "/hotel", enabled: services.hotel },
    { name: "Car", href: "/car", enabled: services.car },
    { name: "Tour", href: "/tour", alwaysShow: true },
    { name: "Blog", href: "/blog", alwaysShow: true },
  ];

  return (
    <>
      <div className={cn("inset-x-0 w-full md:flex hidden bg-fourth mx-auto lg:px-32 md:px-18 px-10 z-50", className)}>
        <div className="w-full h-full flex flex-row justify-between items-center">
          <div className="w-1/4 h-full">
            <Link href="/" className="w-full h-full flex gap-1 justify-center items-center">
              <Image src="/logo.png" width={70} height={70} alt="logo" className="w-12 h-12" />
          ...
          <div className="hidden md:flex font-semibold">
            <Menu setActive={setActive}>
              {loading ? (
                ...
                navItems.map((item) => (
                  (item.alwaysShow || item.enabled) && (
                    <Link key={item.name} href={item.href} className="hover:text-second">
                      {item.name}
                    </Link>
                  )
                ))
